...
TEST_CASE("Full Workflow Integration Tests", "[integration]") {
    ...
    SECTION("Complete XEP-17 token transfer workflow") {
        ...
        auto balance = epicchainToken->balanceOf(sender);
        REQUIRE(balance > 0);
        ...
        scriptBuilder.contractCall(epicchainToken->getScriptHash(), "transfer", {
    ...
    SECTION("Smart contract deployment workflow") {
        ...
        builder.script(scriptBuilder.toArray())
               ...
               .additionalSystemFee(1000000000); // ExtraEpicPulsefor deployment
    ...
    SECTION("NEO voting workflow") {
        ...
        auto epicchainToken = std::make_shared<epicchainToken>();
        auto registerBuilder = epicchainToken->registerCandidate(candidatePublicKey);
        ...
        auto voteBuilder = epicchainToken->vote(voter, candidatePublicKey);
        ...
        auto accountState = epicchainToken->getAccountState(voter);
    ...
    SECTION("Oracle request workflow") {
        ...
        scriptBuilder.contractCall(oracleContract, "request", {
            ...
            ContractParameter::integer(10000000) // epicpulse for callback
        });
    ...
    SECTION("Full token ecosystem workflow") {
        ...
        auto dist1 = epicpulseToken->transfer(treasury, user1->getScriptHash(), 100 * 100000000);
        auto dist2 = epicpulseToken->transfer(treasury, user2->getScriptHash(), 50 * 100000000);
        
        REQUIRE(dist1 != nullptr);
        REQUIRE(dist2 != nullptr);
        
        // 4. User-to-user transfer
        auto userTransfer = epicpulseToken->transfer(user1, user2->getScriptHash(), 25 * 100000000);
