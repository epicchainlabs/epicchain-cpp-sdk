...
#include "epicchaincpp/contract/contract_manifest.hpp"
...
TEST_CASE("ContractManifest Tests", "[contract]") {
    ...
    SECTION("Add supported standards") {
        ...
        manifest.addSupportedStandard("XEP-17");
        manifest.addSupportedStandard("XEP-11");
        
        auto standards = manifest.getSupportedStandards();
        REQUIRE(standards.size() == 2);
        REQUIRE(standards[0] == "XEP-17");
        REQUIRE(standards[1] == "XEP-11");
    }
    ...
    SECTION("Check supported standards") {
        ...
        REQUIRE(manifest.supportsStandard("XEP-17") == true);
        REQUIRE(manifest.supportsStandard("XEP-11") == true);
        REQUIRE(manifest.supportsStandard("XEP-5") == false);
        REQUIRE(manifest.supportsStandard("XEP-10") == false);
    }
    ...
    SECTION("Convert to JSON") {
        ...
        REQUIRE(manifestJson["supportedstandards"][0] == "XEP-17");
    ...
    SECTION("Parse from JSON") {
        json manifestJson = {
            ...
            {"supportedstandards", json::array({"XEP-17", "XEP-11"})},
            {"abi", {
                {"methods", json::array()},
                {"events", json::array()}
            }},
    ...
    SECTION("Constructor from JSON") {
        json manifestJson = {
            ...
            {"groups", json::array()},
            {"supportedstandards", json::array({"XEP-17"})},
            {"abi", {
    ...
    SECTION("Complex manifest roundtrip") {
        ...
        original.addSupportedStandard("XEP-17");
        original.addSupportedStandard("XEP-11");
