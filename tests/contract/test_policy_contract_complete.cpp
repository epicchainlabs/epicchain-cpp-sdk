...
#include "epicchaincpp/contract/policy_contract.hpp"
...
TEST_CASE("PolicyContract Complete Tests", "[contract]") {
    ...
    SECTION("Get fee per byte") {
        ...
        // Default is 1000 (0.00001EpicPulseper byte)
        REQUIRE(feePerByte >= 0);
    }
    
    SECTION("Set fee per byte") {
        int64_t newFeePerByte = 2000; // 0.00002EpicPulseper byte
    ...
    SECTION("Get storage price") {
        ...
        // Result would come from RPC call
        // Default is 100000 (0.001EpicPulseper byte)
        REQUIRE(storagePrice >= 0);
    }
    
    SECTION("Set storage price") {
        uint32_t newStoragePrice = 200000; // 0.002EpicPulseper byte
    ...
    SECTION("Fee calculations") {
        ...
        int64_t feePerByte = 1000; // 0.00001EpicPulseper byte
