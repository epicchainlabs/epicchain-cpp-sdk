...
TEST_CASE("Response Complete Tests", "[protocol]") {
    ...
    SECTION("Parse invokeFunction response") {
        ...
        REQUIRE(response.getResult()["epicpulseConsumed"] == "2007570");
    ...
    SECTION("Parse getNep17Balances response") {
        ...
        REQUIRE(response.getResult()["balance"][0]["symbol"] == "XPR");
        REQUIRE(response.getResult()["balance"][1]["symbol"] == "XPP");
    }
    ...
    SECTION("Parse getContractState response") {
        json contractResult = {
            ...
            {"nef", {
                {"magic", 860243278},
                {"compiler", "epicchain-core-v.0"},
                {"source", "https://github.com/epicchainlabs/epicchain"},
                {"tokens", json::array()},
                {"script", "base64script"},
                {"checksum", 1234567890}
            }},
        ...
        REQUIRE(response.getResult()["manifest"]["name"] == "EpicChainToken");
        REQUIRE(response.getResult()["manifest"]["supportedstandards"][0] == "XEP-17");
    }
