...
#include "epicchaincpp/protocol/core/request.hpp"
...
TEST_CASE("RPC Integration Tests", "[protocol][integration]") {
    ...
    SECTION("XEP-17 transfers simulation") {
        // Register handler for getxep17transfers
        mockClient->registerHandler("getxep17transfers", [](const Request& req) {
            if (req.getParams().empty()) {
                return std::make_shared<Response>(
                    Response::Error(-32602, "Invalid params")
                );
                ...
                {"received", {
                    {
                        ...
                        {"transferaddress", "XZNos2WqTbu5oCgyfss9kUJgBXJqhuYAaj"},
                        {"amount", "100"},
                        {"blockindex", 500},
        ...
        Request request = Request::getXep17Transfers("XZNos2WqTbu5oCgyfss9kUJgBXJqhuYAaj");
        auto response = mockClient->send(request);
