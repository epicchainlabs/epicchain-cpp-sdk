...
TEST_CASE("Wallet Tests", "[wallet]") {
    ...
    SECTION("Import account from XEP-2") {
        Wallet wallet;
        
        // Use a valid XEP-2 format (this is just an example)
        std::string xep2 = "6PYLHmDf6AjF4AsVtosmxHuPYeuyJL3SLuw7J1U8i7HxKAnYNsp61HYRfF";
        std::string password = "TestPassword";
        
        auto account = wallet.importFromXEP2(xep2, password, "XEP-2 Account");
        
        if (account) { // XEP-2 import might not be implemented
            REQUIRE(wallet.size() == 1);
            REQUIRE(wallet.containsAccount(account->getAddress()) == true);
        }
    }
