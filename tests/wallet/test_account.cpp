...
TEST_CASE("Account Tests", "[wallet]") {
    ...
    SECTION("Create account from key pair") {
        ...
        REQUIRE(account->getAddress()[0] == 'N'); // EpicChain address prefix
    ...
    SECTION("Create account from XEP-2") {
        ...
        // Create XEP-2 encrypted key
        std::string xep2 = XEP2::encrypt(keyPair, password);
        
        // Create account from XEP-2
        auto account = Account::fromXEP2(xep2, password, "XEP2Account");
        
        REQUIRE(account->getLabel() == "XEP2Account");
        REQUIRE(!account->getAddress().empty());
    ...
    SECTION("Export account as XEP-2") {
        auto account = Account::create("XEP2ExportAccount");
        const std::string password = "ExportPassword";
        
        std::string xep2 = account->exportXEP2(password);
        REQUIRE(!xep2.empty());
        REQUIRE(XEP2::isValid(xep2));
        
        // Should be able to recreate account from XEP-2
        auto recreated = Account::fromXEP2(xep2, password);
        REQUIRE(recreated->getAddress() == account->getAddress());
    }
