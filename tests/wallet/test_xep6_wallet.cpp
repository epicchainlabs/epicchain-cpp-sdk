...
TEST_CASE("Xep6Wallet Tests", "[wallet]") {
    
    SECTION("Create empty XEP-6 wallet") {
        Xep6Wallet wallet("XEP-6 Test Wallet", "1.0");
        
        REQUIRE(wallet.getName() == "XEP-6 Test Wallet");
    ...
    SECTION("Create XEP-6 wallet with default values") {
        Xep6Wallet wallet;
        
        REQUIRE(wallet.getName() == "EpicChainCpp Wallet");
        REQUIRE(wallet.getVersion() == "1.0");
        REQUIRE(wallet.isEmpty() == true);
    ...
    SECTION("Convert wallet to JSON") {
        Xep6Wallet wallet("JSON Test Wallet", "1.0");
    ...
    SECTION("Create wallet from JSON") {
        ...
        auto wallet = Xep6Wallet::fromJson(walletJson);
    ...
    SECTION("Save and load XEP-6 wallet") {
        std::string filepath = "/tmp/test_nep6_wallet.json";
        
        // Create and save wallet
        {
            Xep6Wallet wallet("Persistent XEP-6", "1.0");
        ...
        // Load wallet
        {
            auto loadedWallet = Xep6Wallet::load(filepath);
            
            REQUIRE(loadedWallet != nullptr);
            REQUIRE(loadedWallet->getName() == "Persistent XEP-6");
    ...
    SECTION("Save and load encrypted XEP-6 wallet") {
        ...
        {
            Xep6Wallet wallet("Encrypted XEP-6", "1.0");
        ...
        {
            auto wrongWallet = Xep6Wallet::load(filepath, "WrongPassword");
        ...
        {
            auto correctWallet = Xep6Wallet::load(filepath, password);
            
            if (correctWallet) {
                REQUIRE(correctWallet->getName() == "Encrypted XEP-6");
    ...
    SECTION("XEP-6 JSON structure validation") {
        Xep6Wallet wallet("Structure Test", "1.0");
        
        auto account = wallet.createAccount("Test Account");
        
        json walletJson = wallet.toJson();
        
        // Check required XEP-6 fields
        REQUIRE(walletJson.contains("name"));
        REQUIRE(walletJson.contains("version"));
        REQUIRE(walletJson.contains("accounts"));
        ...
        if (!walletJson["accounts"].empty()) {
            ...
            // XEP-6 account should have these fields
    ...
    SECTION("Import accounts from JSON with encrypted keys") {
        ...
        auto wallet = Xep6Wallet::fromJson(walletJson, password);
        
        if (wallet) {
            REQUIRE(wallet->getName() == "Import Test");
            // Account import might fail if XEP-2 decryption not implemented
            if (wallet->size() > 0) {
    ...
    SECTION("Multiple token management") {
        ...
        bool hasEpicChain = false, hasGas = false, hasFlm = false;
        for (const auto& token : wallet.getTokens()) {
            if (token["symbol"] == "XPR") hasEpicChain = true;
            if (token["symbol"] == "XPP") hasEpicPulse = true;
            if (token["symbol"] == "XUSD") hasEpicDollar = true;
        }
        
        REQUIRE(hasEpicChain == true);
        REQUIRE(hasEpicPulse == true);
        REQUIRE(hasEpicDollar == true);
    }
    ...
    SECTION("Wallet JSON round-trip") {
        Xep6Wallet original("Round Trip Test", "2.0");
        ...
        auto restored = Xep6Wallet::fromJson(walletJson);
    ...
    SECTION("Empty wallet JSON") {
        Xep6Wallet emptyWallet;
        
        json walletJson = emptyWallet.toJson();
        
        REQUIRE(walletJson["name"] == "EpicChainCpp Wallet");
        REQUIRE(walletJson["version"] == "1.0");
        REQUIRE(walletJson["accounts"].is_array());
    ...
    SECTION("Invalid JSON handling") {
        ...
        auto wallet1 = Xep6Wallet::fromJson(invalidJson1);
        ...
        auto wallet2 = Xep6Wallet::fromJson(invalidJson2);
        ...
        auto wallet3 = Xep6Wallet::fromJson(invalidJson3);
        // Should handle gracefully
    }
    
    SECTION("XEP-6 compatibility with base Wallet class") {
        // XEP-6 wallet should work with all base wallet functionality
        Xep6Wallet xep6Wallet("XEP-6", "1.0");
        Wallet* baseWallet = &xep6Wallet;
        ...
        // XEP-6 specific features should still work
        json token = {{"symbol", "TEST"}, {"decimals", 8}};
        xep6Wallet.addToken(token);
        
        REQUIRE(xep6Wallet.getTokens().size() == 1);
    }
}