...
TEST_CASE("AddressUtils Tests", "[utils]") {
    ...
    SECTION("Create address from script hash") {
        ...
        REQUIRE(address[0] == 'N'); // EpicChain addresses start with 'N'
        REQUIRE(address.length() == 34); // Standard EpicChain address length
    }
    ...
    SECTION("Known address test vectors") {
        // Test with known EpicChain addresses
    ...
    SECTION("Address version byte") {
        ...
        // EpicChain mainnet version
        REQUIRE(version == 0x35); // EpicChain uses 0x35 (53 decimal)
    }
}