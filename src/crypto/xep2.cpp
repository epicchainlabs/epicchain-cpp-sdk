...
std::string XEP2::encrypt(const Bytes& privateKey, const std::string& password, const ScryptParams& params) {
    if (privateKey.size() != 32) {
        throw XEP2Exception("Private key must be 32 bytes");
    }
    ...
    // Build XEP-2 encrypted key
    Bytes xep2;
    xep2.push_back(XEP2_PREFIX_1);
    xep2.push_back(XEP2_PREFIX_2);
    xep2.push_back(XEP2_FLAG);
    xep2.insert(xep2.end(), salt.begin(), salt.end());
    xep2.insert(xep2.end(), encrypted1.begin(), encrypted1.end());
    xep2.insert(xep2.end(), encrypted2.begin(), encrypted2.end());
    
    return Base58::encodeCheck(xep2);
}

std::string XEP2::encrypt(const ECKeyPair& keyPair, const std::string& password, const ScryptParams& params) {
    return encrypt(keyPair.getPrivateKey()->getBytes(), password, params);
}

Bytes XEP2::decrypt(const std::string& xep2, const std::string& password, const ScryptParams& params) {
    if (!isValid(xep2)) {
        throw XEP2Exception("Invalid XEP-2 format");
    }
    
    Bytes encrypted = Base58::decodeCheck(xep2);
    if (encrypted.size() != XEP2_ENCRYPTED_SIZE) {
        throw XEP2Exception("Invalid XEP-2 length");
    }
    ...
    try {
        ...
        for (size_t i = 0; i < 4; ++i) {
            if (salt[i] != addressHash[i]) {
                throw XEP2Exception("Invalid password");
            }
        }
        
        return privateKey;
    } catch (const XEP2Exception&) {
        throw;
    } catch (...) {
        throw XEP2Exception("Invalid password or corrupted XEP-2 key");
    }
}

ECKeyPair XEP2::decryptToKeyPair(const std::string& xep2, const std::string& password, const ScryptParams& params) {
    Bytes privateKey = decrypt(xep2, password, params);
    return ECKeyPair(privateKey);
}

bool XEP2::isValid(const std::string& xep2) {
    if (xep2.length() != 58) {
        return false;
    }
    
    try {
        Bytes decoded = Base58::decodeCheck(xep2);
        if (decoded.empty()) {
            return false;
        }
        return decoded.size() == XEP2_ENCRYPTED_SIZE &&
               decoded[0] == XEP2_PREFIX_1 &&
               decoded[1] == XEP2_PREFIX_2 &&
               decoded[2] == XEP2_FLAG;
    } catch (...) {
        return false;
    }
...
std::string XEP2::getAddress(const std::string& xep2) {
    if (!isValid(xep2)) {
        throw XEP2Exception("Invalid XEP-2 format");
    }
    
    // XEP-2 doesn't directly contain the address
    // We'd need to decrypt to get it
    throw XEP2Exception("Cannot get address without decryption");
}
