...
#include "epicchaincpp/script/verification_script.hpp"
#include <random>
#include <algorithm>
...
TransactionBuilder& TransactionBuilder::calculateNetworkFee() {
    ...
    for (const auto& signer : transaction_->getSigners()) {
        ...
        fee += 1000000; // 0.01EpicPulsebase cost
    }
...
TransactionBuilder& TransactionBuilder::transferGas(const SharedPtr<Account>& from, 
                                                    const std::string& to, 
                                                    int64_t amount) {
    return transferNep17(EpicPulseToken::SCRIPT_HASH, from, to, amount, 8);
}
...
int64_t TransactionBuilder::getSystemFeeForScript() {
    ...
    return invokeResult.epicpulseConsumed;
}
...
int64_t TransactionBuilder::getSenderGasBalance() {
    ...
    // Call balanceOf on theEpicPulsecontract
    std::vector<ContractParameter> params = {
    ...
    auto response = client_->invokeFunction(EpicPulseToken::SCRIPT_HASH, "balanceOf", jsonParams);
    if (!response) {
        throw RuntimeException("Failed to getEpicPulsebalance");
    }
