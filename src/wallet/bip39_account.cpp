#include "epicchaincpp/wallet/bip39_account.hpp"
...
std::string Bip39Account::exportXEP2(const std::string& password) const {
    if (!getKeyPair()) {
        throw std::runtime_error("No private key available for XEP2 export");
    }
    ...
    try {
        ...
        // Encrypt using XEP2 format
        return XEP2::encrypt(privateKeyBytes, password);
        
    } catch (const std::exception& e) {
        std::stringstream ss;
        ss << "Failed to export account to XEP2: " << e.what();
        throw std::runtime_error(ss.str());
    }
}
