...
Account::Account(const std::string& xep2, const std::string& password, const std::string& label)
    : label_(label),
      isDefault_(false),
      isLocked_(true),
      encryptedPrivateKey_(xep2) {
    // Decrypt to get address
    auto tempKeyPair = std::make_shared<ECKeyPair>(XEP2::decryptToKeyPair(xep2, password));
...
void Account::lock(const std::string& password) {
    ...
    encryptedPrivateKey_ = XEP2::encrypt(*keyPair_, password);
    keyPair_ = nullptr;
    isLocked_ = true;
}
...
bool Account::unlock(const std::string& password) {
    ...
    try {
        keyPair_ = std::make_shared<ECKeyPair>(XEP2::decryptToKeyPair(encryptedPrivateKey_, password));
        isLocked_ = false;
        return true;
...
std::string Account::exportXEP2(const std::string& password) const {
    if (!keyPair_) {
    ...
    return XEP2::encrypt(*keyPair_, password);
}
...
SharedPtr<Account> Account::fromXEP2(const std::string& xep2, const std::string& password, const std::string& label) {
    return std::make_shared<Account>(xep2, password, label);
}
