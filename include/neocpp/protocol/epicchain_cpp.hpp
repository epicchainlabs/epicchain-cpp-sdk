...
/// Configuration for EpicChainCpp client
struct EpicChainCppConfig {
    /// Default block time in milliseconds (15 seconds on EpicChain)
    ...
    /// The interval in milliseconds in which EpicChainCpp should poll the Neo node for new block information
...
/// Main EpicChainCpp client class for interacting with Neo blockchain
class EpicChainCpp {
private:
    EpicChainCppConfig config_;
...
public:
    ...
    EpicChainCpp(const EpicChainCppConfig& config, SharedPtr<HttpService> httpService);
    
    /// Constructor with URL only (uses default configuration)
    /// @param url The RPC endpoint URL
    explicit EpicChainCpp(const std::string& url);
    
    /// Static factory method to build a EpicChainCpp instance
    /// @param httpService The HTTP service to use
    /// @param config The configuration (optional, defaults to new one)
    /// @return A new EpicChainCpp instance
    static SharedPtr<epicchaincpp> build(SharedPtr<HttpService> httpService, const EpicChainCppConfig& config = EpicChainCppConfig());
    
    /// Static factory method to build a EpicChainCpp instance from URL
    /// @param url The RPC endpoint URL
    /// @param config The configuration (optional, defaults to new one)
    /// @return A new EpicChainCpp instance
    static SharedPtr<epicchaincpp> build(const std::string& url, const EpicChainCppConfig& config = EpicChainCppConfig());
    
    // Configuration getters
    
    /// Get the configuration
    const EpicChainCppConfig& getConfig() const { return config_; }
    ...
    int getBlockInterval() const { return config_.blockInterval; }
    
    /// Get the polling interval in milliseconds
    int getPollingInterval() const { return config_.pollingInterval; }
    
    /// Get the maximum valid until block increment
    int getMaxValidUntilBlockIncrement() const { return config_.maxValidUntilBlockIncrement; }
    ...
    /// Get XEP-17 balances for an address
    /// @param address The address to query
    /// @return The XEP-17 balances
