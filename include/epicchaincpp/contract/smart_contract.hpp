...
public:
    ...
    std::vector<std::string> getSupportedStandards();
    
    /// Get ABI
    /// @return The contract ABI
    nlohmann::json getAbi();
    
    /// Get contract methods
    /// @return List of contract methods
    std::vector<nlohmann::json> getMethods();
...
protected:
    ...
    nlohmann::json paramsToJson(const std::vector<ContractParameter>& params);
};
