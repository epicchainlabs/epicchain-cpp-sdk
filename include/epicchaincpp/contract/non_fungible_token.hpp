...
/// Base class for XEP-11 non-fungible token contracts
...
public:
    ...
    int64_t getTotalSupply();
    
    /// Get balance of an address
    /// @param address The address
    /// @return The balance (number of NFTs owned)
    int64_t getBalanceOf(const std::string& address);
    ...
    /// Check if divisible (XEP-11 divisible standard)
    ...
    int64_t getBalanceOf(const std::string& address, const std::string& tokenId);
